...
class FeatureMap(nn.Module):
    ...
    def __init__(self, alpha = 1.0, config=None, img_size = (480,640)):
        ...
        if alpha == 1.0:
            MNASNet = torchvision.models.mnasnet1_0(pretrained= True, progress =False)
        else:
...
class Transformer_3D(nn.Module):
    ...
    def __init__(self, model_name):
        ...
        self.conv = nn.Conv2d(out_c[0], out_c[1], 1)

        self.deconv0 = nn.Sequential(nn.ConvTranspose2d(out_c[1], out_c[1], 5 , 2, 2, 1, bias = False))
    ...
    def forward(self, img):
        ...
        T_F = self.Transformer(feature_map)
        T_img = self.Transformer(TF.resize(img,(224,224)))
        
        T_output = T_F + T_img
        T_output = T_output.view(B,27,809)
        

        out = self.conv(self.linear(T_output).view(B, -1, 15,20))
