...
from network.Transformer_3d import *
...
class NeuralRecon(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Network
        self.Transformer3d = Transformer_3D('B_16')
        
        ##self.backbone2d = MnasMulit(alpha)
    ...
    def forward(self, inputs, save_mesh = False):
        ...
        for i in range(len(imgs)):
            tmp = self.Transformer3d(self.normalizer(imgs[i]))#, edge[i].float())
            features.append(tmp)
