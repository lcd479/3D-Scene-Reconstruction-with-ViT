...
class MnasMulit(nn.Module):
    
    def __init__(self, alpha = 1.0):
        super(MnasMulit, self).__init__()
        ...
        self.conv0 = nn.Sequential(MNASNet.layers._modules['0'],
        ...
        self.out_channels.append(detphs[3])
        self.out_channels.append(detphs[2])
    ...
    def forward(self, x):
        ...
        intra_feat = conv2_out
        outputs = []
        
        out = self.out1(intra_feat)
        outputs.append(out)
        
        intra_feat = F.interpolate(intra_feat, scale_factor=2, mode= "nearest") + self.inner1(conv1_out)
        out = self.out2(intra_feat)
        outputs.append(out)
        
        intra_feat = F.interpolate(intra_feat, scale_factor=2, mode= "nearest") + self.inner2(conv0_out)
